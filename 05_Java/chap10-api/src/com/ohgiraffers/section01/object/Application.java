...
public class Application {
    public static void main(String[] args) {
        ...
        Book book2 = new Book(2,"목민심서", "정약용", 20000);
        Book book3 = new Book(2, "목민심서", "정약용", 20000);
        ...
        * 하지만 때로는 주소가 달라도 필드의 내요이 같으면 '같다(동등하다)'고 판단해야 할 때가 있다.
        * 이런 '동등성 비교'를 위해 equals()를 오버라이딩 한다.
        * */
        System.out.println("동일성 비교(==) : " +(book2 == book3)); //주소값이 같은지
        System.out.println("동등성 비교(equals) : " + book2.equals(book3)); //값이 같은지

        // hashcode : 객체를 숫자로 변환한 값(컴퓨터가 빠르게 찾기 위한)
        // 자바의 중요한 규약: equals() 비교로 true를 반환한다면,
        // 두 객체의 hashCode() 값은 반드시 같아야 한다.
        /**
         * HashSet, HashMap 같은 해시 기반 컬렉션은 객체를 찾을 때 다음 순서로 찾는다.
         * 1. hashCode()로 어느 칸을 찾아볼지 먼저 결정
         * 2. 같은 칸 안에서 equals()로 정말 같은 객체인지 최종 비교
         * 만약 equals()는 true인데 hashCode가 다르면, 애초에 다른 칸을 보게 되므로 equals() 비교까지 도달하지 못함.
         */
